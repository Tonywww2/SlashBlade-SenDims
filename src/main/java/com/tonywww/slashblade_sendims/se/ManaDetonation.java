...
public class ManaDetonation extends SpecialEffect {
    ...
    public static void onSlashBladeUpdate(ServerPlayer player, SlashBladeEvent.UpdateEvent event) {
        ...
        if (level.getGameTime() % 20 == 0 && event.getBlade().getDamageValue() > 0) {
            if (ManaItemHandler.instance().requestManaExactForTool(BotaniaItems.terraSword.getDefaultInstance(), player, 100, true)) {
        ...
        if (level.getGameTime() % 100 == 0) {
            ...
            if (storedDamage >= attackDamage * 2.0) {
                ...
                if (!level.isClientSide) {
                    ...
                    List<LivingEntity> targets = level.getEntitiesOfClass(LivingEntity.class, aabb, e -> e != player && e.isAlive());
    ...
    private static void spawnManaParticles(Player player) {
        float r = 0.2f;
        float g = 0.8f;
        float b = 1.0f;
    ...
    public static void onLivingHurt(LivingHurtEvent event, ItemStack bladeStack, boolean isMagic, int resonanceLevel) {
        if (!isMagic) {
            float originalDamage = event.getAmount();
            event.setAmount(originalDamage * Math.min(1f, 0.5f + (resonanceLevel / 200f)));
